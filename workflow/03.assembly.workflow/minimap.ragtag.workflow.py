...
def load_workflow(workflow, sample):
    ...
    OUTPUT_DIR = f'/data/home/sjwan/projects/Y-chromosome/workflow.output/03.assembly.workflow/{sample}/T2TY.minimap'
    os.system(f'rm -rf {OUTPUT_DIR}')
    check_and_make_path(OUTPUT_DIR)
    PAF_OUTPUT = join_path(OUTPUT_DIR, 'minimap.paf')
    minimap_ragtag2reference = Minimap2_asm20(logger)
    minimap_ragtag2reference.RUN_PARAMS['target'] = T2T_Y
