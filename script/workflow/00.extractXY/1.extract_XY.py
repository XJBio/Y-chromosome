...
def add_minimap2_T2TY(workflow, sample, out_dir):

    # 路径
    assembly_fa = f"/data/home/sjwan/projects/Y-chromosome/workflow.output/data/verkko1.4/{sample}/assembly.fasta"
    T2T_Y = "/data/home/sjwan/projects/Y-chromosome/workflow.output/data/T2TY.fa"
    out_paf = join_path(out_dir, "minimap2.asm5.Y.paf")

    # minimap2 = Minimap2_asm20(logger)
    minimap2 = Minimap2_asm5(logger)
    minimap2.RUN_PARAMS["threads"] = "48"
    minimap2.RUN_PARAMS["target"] = T2T_Y
    minimap2.RUN_PARAMS["query"] = assembly_fa
    minimap2.RUN_PARAMS["output"] = out_paf

    workflow.add_software(minimap2)


def add_minimap2_T2TX(workflow, sample, out_dir):

    # 路径
    assembly_fa = f"/data/home/sjwan/projects/Y-chromosome/workflow.output/data/verkko1.4/{sample}/assembly.fasta"
    T2T_X = "/data/home/sjwan/projects/Y-chromosome/workflow.output/data/T2TX.fa"
    out_paf = join_path(out_dir, "minimap2.asm5.X.paf")

    # minimap2 = Minimap2_asm20(logger)
    minimap2 = Minimap2_asm5(logger)
    minimap2.RUN_PARAMS["threads"] = "48"
    minimap2.RUN_PARAMS["target"] = T2T_X
    minimap2.RUN_PARAMS["query"] = assembly_fa
    minimap2.RUN_PARAMS["output"] = out_paf
...
if __name__ == "__main__":
    ...
    for sample in sample_list:
        ...
        MINIMAP2_OUTPUT = join_and_make_path(output_dir, "ALL.minimap2XY")
        # add_minimap2_T2TY(sample_flow, sample, MINIMAP2_OUTPUT)
        add_minimap2_T2TX(sample_flow, sample, MINIMAP2_OUTPUT)
        HMMER_OUTPUT = join_and_make_path(output_dir, "ALL.HMMER")
        motif_list = ["DYZ1_Yq", "DYZ18_Yq", "DYZ3-sec_Ycentro", "DYZ2_Yq"]
